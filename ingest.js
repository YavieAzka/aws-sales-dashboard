...
async function main() {
  ...
  console.log("Step 1: Reading and collecting data from CSV...");
  const { outletMap, productMap, salesData } = await processCsv();

  // --- FASE 2: BATCH INSERT OUTLET & PRODUK ---
  console.log(
    `Step 2: Inserting ${outletMap.size} unique outlets and ${productMap.size} unique products...`
  );
...
function processCsv() {
  return new Promise((resolve, reject) => {
    ...
    const csvFilePath = path.join(scriptDir, "data-miniature.csv"); // Pastikan ini nama file 100rb baris Anda
